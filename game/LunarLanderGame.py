...
class LunarLanderGame:
    ...
    def get_possible_moves(self, current_position, figures):
        ...
        while nx >= 0 and (nx, y) not in figures.values():
            nx -= 1
        if nx >= 0 and (nx, y) in figures.values():
            moves.append((nx + 1, y))
        ...
        while nx < self.board_size and (nx, y) not in figures.values():
            nx += 1
        if nx < self.board_size and (nx, y) in figures.values():
            moves.append((nx - 1, y))
        ...
        while ny >= 0 and (x, ny) not in figures.values():
            ny -= 1
        if ny >= 0 and (x, ny) in figures.values():
            moves.append((x, ny + 1))
        ...
        while ny < self.board_size and (x, ny) not in figures.values():
            ny += 1
        if ny < self.board_size and (x, ny) in figures.values():
            moves.append((x, ny - 1))
